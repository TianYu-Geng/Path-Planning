...
使用曼哈顿距离但地图上障碍太多、绕路严重，h 严重低估，A* 的估计失真。
"""
...
class AStar:
    ...
    def __init__(self, s_start, s_goal, heuristic_type, heuristic_weight=1.0):
        ...
        # A* 只允许 4 方向移动（不允许对角线，与Dijkstra保持一致）
        # 上、下、左、右
...
def main():
    ...
    print("【对比总结】")
    print(f"{'='*100}")
    print(f"{'情况':<30} {'权重':<10} {'访问节点':<12} {'路径长度':<12} {'总代价':<12}")
    print("-" * 100)
    for r in results:
    ...
    print(f"\n分析:")
    print(f"  1. h(n)=0 (w=0.0): 等价 Dijkstra，访问节点最多，保证最优")
    print(f"  2. 标准A* (w=1.0): 平衡效率和最优性，访问节点适中，保证最优")
    print(f"  3. Weighted A* (w=2.5): 更快但可能不是最优，访问节点最少")
    print("=" * 100)
